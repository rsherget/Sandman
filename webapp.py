...
def home():
    ...
    shifts = shiftHelper.organizeShifts(tables.getUserShifts(current_user.id), thisYear, thisMonth)
    if request.method == 'POST':
        if "next" in request.form:
            if datetime.datetime.today().month == 12:
                shifts = shiftHelper.organizeShifts(tables.getUserShifts(current_user.id, year=thisYear+1, month=1), thisYear + 1, 1)
            else:
                shifts = shiftHelper.organizeShifts(tables.getUserShifts(current_user.id, year=thisYear, month=thisMonth+1), thisYear, thisMonth+1)
